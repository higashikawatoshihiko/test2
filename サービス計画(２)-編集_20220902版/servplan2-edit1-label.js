...
document.addEventListener('click', ev => {
  const pattern = '#servplan-table td';
  const currentTarget = ev.target.matches(pattern) ? ev.target : ev.target.closest(pattern);
  if (currentTarget) {
    // 選択セルの解除
    for (const cell of document.querySelectorAll('#servplan-table td[data-active]')) {
    ...
    // テキストエリアのリサイズ
    for (const textarea of currentTarget.querySelectorAll('textarea.auto-height')) {
      setTimeout(() => resizeTextAreaHeight(textarea), 1);
    }
  }
});
...
$(document).on('click', '.btn-row-add', function(e) {
  // 操作対象セル
  let cell = $('#servplan-table td[data-active=true]');
...
$(document).on('click', '.btn-row-delete', async function(e) {
  ...
  // 行の単純削除
  if (getRowGroupName(row) === groupName && !(groupName === 'needs' && row.is('tr.row-needs:first'))) {
    // 行結合操作
...
// 行グループ階層
GROUPS = ['needs', 'long', 'short', 'service', 'service-type'];
